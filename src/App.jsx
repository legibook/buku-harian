...
import { 
  BookHeart, PenLine, CalendarHeart, ChevronLeft, Sparkles, Moon, Sun, Sunset, 
  Menu, Trash2, Edit3, X, RotateCcw, ArchiveX, Bold, Italic, Underline, Palette, 
  Search, Quote, Plus, GripVertical, Tag, Settings, Check, FileText, Type 
} from 'lucide-react';
...
const App = () => {
  ...
  const [labels, setLabels] = useState(() => JSON.parse(localStorage.getItem('diary-muslimah-labels')) || [
    { id: 1, name: 'Penting ✨' }, { id: 2, name: 'Keluarga 👨‍👩‍👧' }, { id: 3, name: 'Ibadah 🕋' }
  ]);
  ...
  // Logika Filter Jurnal (Live Search)
  const displayedEntries = useMemo(() => entries.filter(entry => {
    const s = searchTerm.toLowerCase();
    const matchSearch = entry.title.toLowerCase().includes(s) || entry.content.toLowerCase().includes(s);
    const matchLabel = selectedLabelFilter ? (entry.labels?.includes(selectedLabelFilter)) : true;
    return matchSearch && matchLabel;
  }), [entries, searchTerm, selectedLabelFilter]);

  // Fungsi Pembantu Waktu & Ikon Cuaca
  ...
  const resetForm = useCallback(() => { 
    ...
    setNewMood('senang'); 
    setNewEntryLabels([]); 
    setNewPaper('paper-polos'); 
    setNewFont('font-sans');
  }, []);

  // Fungsi Aksi Quote (Drag & Drop)
  ...
  const handleDeleteQuote = useCallback((i) => { 
    const u = [...userQuotes]; 
    u.splice(i, 1); 
    setUserQuotes(u); 
  }, [userQuotes]);
    ...
    return (
      <div className="flex flex-wrap gap-1.5 mt-2 mb-1">
        {entryLabelsArray.map(id => {
          ...
          return l ? (
            <span key={id} className={`text-[10px] border ${mode.border} px-2 py-0.5 rounded-full font-medium ${isDarkMode ? 'bg-gray-800 text-gray-300' : `${theme.bgLight} ${theme.text}`}`}>
              {l.name}
            </span>
          ) : null;
        })}
  ...
  // --- GAYA INLINE UNTUK KERTAS (PRESISI & AMAN DARI BOCOR CSS) ---
  const getPaperStyle = (paperType) => {
    const baseStyle = { padding: '1rem' };
    
    // Warna garis yang estetik (dusty pink pudar)
    const lineColor = 'rgba(220, 150, 180, 0.4)';
    const dotColor = 'rgba(220, 150, 180, 0.6)';
    const gridColor = 'rgba(220, 150, 180, 0.3)';

    switch(paperType) {
      case 'paper-garis':
        return {
          ...baseStyle,
          backgroundImage: `linear-gradient(transparent calc(1.6em - 1px), ${lineColor} calc(1.6em - 1px), ${lineColor} 1.6em, transparent 1.6em)`,
          backgroundSize: '100% 2em',
          backgroundPosition: '0 1rem', // menyesuaikan padding 1rem
          lineHeight: '2em',
      ...
      case 'paper-titik':
        return {
          ...
          backgroundPosition: '1rem calc(1rem + 0.6em)',
          lineHeight: '2em',
          backgroundAttachment: 'local'
        };
      case 'paper-grid':
        return {
          ...baseStyle,
          backgroundImage: `linear-gradient(to right, ${gridColor} 1px, transparent 1px), linear-gradient(to bottom, transparent calc(1.6em - 1px), ${gridColor} calc(1.6em - 1px), ${gridColor} 1.6em, transparent 1.6em)`,
          backgroundSize: '2em 2em',
      ...
      default: // paper-polos
        return { ...baseStyle, backgroundColor: 'transparent', lineHeight: '1.75em' };
    }
  };

  // Pemetaan Font menggunakan font bawaan sistem agar tidak merusak tata letak
  const getFontClass = (fontType) => {
    switch(fontType) {
      case 'font-elegant': return 'font-serif';
      case 'font-handwriting': return 'font-[cursive] text-lg';
      case 'font-script': return 'font-mono'; // Menggunakan monospace untuk variasi yang aman
  ...
  return (
    <div className={`h-[100dvh] w-full ${mode.bgMain} font-sans ${mode.textMain} relative overflow-hidden transition-colors duration-500 selection:${theme.bgMedium} selection:${theme.text}`}>
      ...
      {/* Sidebar Overlay */}
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-40 transition-opacity" onClick={() => setIsSidebarOpen(false)}></div>
      )}
        ...
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="space-y-3">
            {[
              ...
              <button 
                key={item.id} 
                onClick={() => { setCurrentView(item.id); setSelectedLabelFilter(null); setIsSidebarOpen(false); }} 
                className={`w-full flex items-center gap-3 p-4 rounded-2xl font-medium transition-all ${item.active ? `bg-gradient-to-r ${isDarkMode ? 'from-gray-800 to-gray-700' : theme.gradientLight} ${theme.text} shadow-sm` : `${mode.bgHover} ${mode.textMuted}`}`}
              >
            ...
            <button 
              ...
              className={`w-full flex items-center justify-between p-4 rounded-2xl font-medium transition-all ${currentView === 'trash' ? `bg-gradient-to-r ${isDarkMode ? 'from-gray-800 to-gray-700' : theme.gradientLight} ${theme.text} shadow-sm` : `${mode.bgHover} ${mode.textMuted}`}`}
            >
            ...
            <button 
              ...
              className={`w-full flex items-center gap-3 p-4 rounded-2xl font-medium transition-all ${currentView === 'settings' ? `bg-gradient-to-r ${isDarkMode ? 'from-gray-800 to-gray-700' : theme.gradientLight} ${theme.text} shadow-sm` : `${mode.bgHover} ${mode.textMuted}`}`}
            >
      ...
      <div className={`max-w-md mx-auto h-full ${isDarkMode ? 'bg-black/20' : 'bg-white/40'} backdrop-blur-sm shadow-xl ${isDarkMode ? 'shadow-black/50' : theme.shadow + '/50'} relative z-10 flex flex-col transition-colors duration-500`}>
        ...
        <header className="pt-10 pb-6 px-6 text-center relative flex justify-center items-center shrink-0">
          <button 
            onClick={currentView === 'home' ? () => setIsSidebarOpen(true) : currentView === 'write' ? () => { resetForm(); setCurrentView('home'); } : () => setCurrentView('home')} 
            className={`absolute left-6 p-2 ${mode.bgItem} rounded-full ${mode.bgHover} ${theme.text} transition-colors shadow-sm`}
          >
            {currentView === 'home' ? <Menu size={20} /> : <ChevronLeft size={20} />}
          </button>
          
          <div className="flex flex-col items-center">
            <h1 className={`text-2xl font-serif font-bold bg-clip-text text-transparent bg-gradient-to-r ${theme.gradient} flex items-center gap-2`}>
              <BookHeart className={theme.textLight} size={24} /> Catatan Muslimah
            </h1>
            ...
            <div className="space-y-6 flex flex-col animate-in fade-in slide-in-from-bottom-4 duration-500">
              ...
              <div className="relative mt-2">
                ...
                <input 
                  ...
                  className={`w-full ${mode.bgInput} backdrop-blur-md pl-12 pr-4 py-3 rounded-2xl shadow-sm border ${mode.border} focus:border-transparent focus:outline-none focus:ring-2 ${theme.ring} transition-all text-sm ${mode.textMain}`} 
                />
              </div>
              ...
              {selectedLabelFilter && (
                <div className={`${isDarkMode ? 'bg-gray-800' : theme.bgLight} border ${mode.border} ${theme.text} px-4 py-3 rounded-2xl flex justify-between items-center shadow-sm animate-in fade-in zoom-in`}>
                  ...
                  <button onClick={() => setSelectedLabelFilter(null)} className={`p-1 ${mode.bgHover} rounded-full transition-colors ${theme.text}`}>
                    <X size={16}/>
                  </button>
                </div>
              )}
              ...
              <div className="flex-1 pb-10">
                {!selectedLabelFilter && (
                  <div className="flex items-center justify-between mb-4 mt-2">
                    <h2 className={`text-lg font-bold ${mode.textMain} flex items-center gap-2`}>
                      <CalendarHeart size={18} className={theme.textLight} /> Jurnal Terakhir
                    </h2>
                  </div>
                )}
                
                {entries.length === 0 ? (
                  <div className={`text-center py-10 ${mode.textMuted} flex flex-col items-center`}>
                    <Moon size={40} className={`mb-3 ${theme.textLight} opacity-50`} />
                    <p>Belum ada catatan.</p>
                    <p className="text-sm">Mulai tulis harimu hari ini!</p>
                  </div>
                ) : displayedEntries.length === 0 ? (
                  <div className={`text-center py-8 ${theme.textLight} ${mode.bgInput} rounded-2xl border ${mode.border}`}>
                      ...
                      <div key={entry.id} className={`w-full ${mode.bgCard} backdrop-blur-md rounded-2xl ${mode.shadow} border ${mode.border} hover:shadow-md transition-all group overflow-hidden flex flex-col`}>
                        ...
                        <div className="p-5 cursor-pointer flex-1" onClick={() => { setActiveEntry(entry); setCurrentView('read'); }}>
                          <div className="flex justify-between items-start mb-2">
                            <h3 className={`font-bold ${mode.textMain} group-hover:${theme.text} transition-colors ${getFontClass(entry.font)}`}>
                              {entry.title}
                            </h3>
                            <span className={`text-2xl ${mode.bgItem} w-8 h-8 flex items-center justify-center rounded-full shadow-inner shrink-0 ml-2`}>
                              {moods.find(m => m.id === entry.mood)?.emoji}
                            </span>
                          </div>
                          
                          <div className={`flex items-center gap-1.5 text-xs ${theme.textLight} font-medium mb-1`}>
                          ...
                          <div 
                            className={`text-sm ${mode.textCard} line-clamp-2 leading-relaxed mt-2 ${getFontClass(entry.font)}`} 
          ...
          {currentView === 'write' && (
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col pb-6">
                ...
                <div className={`flex justify-center gap-1 sm:gap-3 mb-3 pb-3 border-b ${mode.border}`}>
                  {moods.map(mood => (
                    <button 
                      ...
                      className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all ${newMood === mood.id ? `${isDarkMode ? 'bg-gray-700' : theme.bgMedium} scale-110 shadow-sm ring-2 ${theme.ring}` : `opacity-70 hover:opacity-100 ${mode.bgHover}`}`}
                    >
                ...
                <div className="px-1">
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    {labels.map(label => (
                      <button 
                        key={label.id} 
                        onClick={() => setNewEntryLabels(newEntryLabels.includes(label.id) ? newEntryLabels.filter(id => id !== label.id) : [...newEntryLabels, label.id])} 
                        className={`px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${newEntryLabels.includes(label.id) ? `bg-gradient-to-r ${theme.gradient} text-white border-transparent shadow-md transform scale-105` : `${mode.bgItem} ${mode.textMuted} ${mode.border} hover:border-gray-400`}`}
                      >
                        {label.name}
                      </button>
                    ))}
                    ...
                    {isAddingInstantLabel ? (
                      <div className={`flex items-center gap-1 ${mode.bgItem} p-1 rounded-full border ${mode.border} shadow-inner`}>
                        <input 
                          ...
                          className={`w-24 text-xs px-2 py-0.5 bg-transparent outline-none ${mode.textMain}`} 
                      ...
                      <button onClick={() => setIsAddingInstantLabel(true)} className={`px-3 py-1.5 rounded-full border border-dashed ${mode.border} ${theme.textLight} ${mode.bgItem} hover:${mode.bgHover} flex items-center gap-1 text-xs font-bold transition-colors`}>
                        <Plus size={12}/> Baru
                      </button>
                    )}
                  </div>
              ...
              <div className="flex flex-wrap gap-4 px-1 shrink-0">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <select value={newFont} onChange={e => setNewFont(e.target.value)} className={`text-xs font-medium ${mode.bgItem} ${mode.textMain} border ${mode.border} rounded-lg py-1 px-2 outline-none focus:ring-2 ${theme.ring} cursor-pointer`}>
                    ...
                    <option value="font-elegant">Elegan</option>
                    <option value="font-handwriting">Tulis Tangan</option>
                    <option value="font-script">Mesin Tik</option>
                  </select>
                </div>
              </div>
              ...
              <div className={`flex-1 flex flex-col ${mode.bgInput} rounded-3xl shadow-inner border ${mode.border} min-h-[300px] overflow-hidden`}>
                ...
                <div className="p-4 pb-0">
                  <input 
                    ...
                    className={`w-full bg-transparent text-xl font-bold ${mode.textMain} ${getFontClass(newFont)} placeholder-gray-400 border-b-2 ${mode.border} focus:border-transparent focus:ring-0 outline-none pb-3 mb-2 transition-colors shrink-0`} 
                  />
                </div>
                ...
                {/* Kanvas Kertas Tulis (Menggunakan Inline Styles Presisi) */}
                <div 
                  ref={editorRef} 
                  contentEditable 
                  ...
                  style={{ 
                    ...
                    ...getPaperStyle(newPaper)
                  }} 
                  className={`w-full flex-1 outline-none ${mode.textCard} overflow-y-auto transition-all ${getFontClass(newFont)}`} 
              ...
              <button 
                ...
                className={`w-full py-4 bg-gradient-to-r ${theme.gradient} text-white rounded-2xl font-bold shadow-lg ${isDarkMode ? 'shadow-black/50' : theme.shadow} transition-all disabled:opacity-50 disabled:cursor-not-allowed transform active:scale-[0.98] shrink-0`}
              >
            ...
            <div className={`${mode.bgCard} backdrop-blur-md rounded-3xl shadow-sm border ${mode.border} animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col`}>
              ...
              <div className="p-6 pb-2">
                ...
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h2 className={`text-2xl font-bold ${mode.textMain} mb-1 ${getFontClass(activeEntry.font)}`}>{activeEntry.title}</h2>
                    <p className={`text-sm ${theme.textLight} font-medium flex items-center gap-2`}>
                      {getTimeIcon(activeEntry.date)}
                      <span>{formatDateTime(activeEntry.date)}</span>
                    </p>
                  </div>
                  <div className={`text-4xl ${mode.bgItem} p-2 rounded-2xl shadow-inner shrink-0 ml-2 border ${mode.border}`}>
              ...
              {/* Area Kertas Baca (Menggunakan Inline Styles Presisi) */}
              <div 
                className={`${mode.textCard} pb-12 transition-all ${getFontClass(activeEntry.font)}`} 
                style={{ 
                  fontSize: `${fontSize}px`,
                  ...getPaperStyle(activeEntry.paper || 'paper-polos')
                }} 
                    ...
                    {userQuotes.map((quote, index) => (
                      <div key={quote.id || index} draggable onDragStart={e => handleDragStart(e, index)} onDragEnter={e => handleDragEnter(e, index)} onDragEnd={handleDragEnd} onDragOver={e => e.preventDefault()} className={`${mode.bgCard} backdrop-blur-sm p-4 rounded-2xl shadow-sm border ${mode.border} flex justify-between items-start gap-3 group cursor-move hover:shadow-md transition-all`}>
                        ...
                        <div className="flex-1">
                          <p className={`text-sm font-serif italic ${mode.textCard}`}>"{quote.text}"</p>
                          <p className={`text-xs font-semibold ${theme.textLight} mt-2`}>— {quote.source}</p>
                        </div>
                    ...
                    {labels.map(label => {
                      ...
                      return (
                        <div key={label.id} className={`${mode.bgCard} backdrop-blur-sm p-4 rounded-2xl shadow-sm border ${mode.border} flex flex-col justify-between hover:shadow-md transition-all`}>
                          <div>
                            <h4 className={`font-bold ${mode.textMain} text-sm mb-1 line-clamp-1`}>{label.name}</h4>
                            <p className={`text-xs font-medium ${theme.textLight}`}>{count} Catatan</p>
                          </div>
                  ...
                  {deletedEntries.map(entry => (
                    <div key={entry.id} className={`w-full ${mode.bgInput} backdrop-blur-md rounded-2xl shadow-sm border ${mode.border} overflow-hidden opacity-80 hover:opacity-100 transition-opacity`}>
                      <div className="p-5 opacity-70">
                        <h3 className={`font-bold ${mode.textMuted} line-through decoration-gray-400 mb-1 ${getFontClass(entry.font)}`}>{entry.title}</h3>
                        ...
                        <div className={`text-sm ${mode.textMuted} line-clamp-1 italic ${getFontClass(entry.font)}`} dangerouslySetInnerHTML={{ __html: entry.content }}></div>
                      </div>
                  ...
                  <div className="flex gap-4 flex-wrap justify-center sm:justify-start">
                    {Object.keys(themeColors).map(t => (
                      <button 
                        ...
                        className={`w-12 h-12 rounded-full shadow-lg bg-gradient-to-br ${themeColors[t].gradient} flex items-center justify-center border-2 transition-all hover:scale-110 ${currentTheme === t ? 'border-white scale-110 ring-4 ' + themeColors[t].ring : 'border-transparent'}`} 
                        title={themeColors[t].name}
                      >
        ...
        {currentView === 'home' && !selectedLabelFilter && (
          <div className="absolute bottom-8 right-6 z-50">
            <button 
              ...
              className={`group flex items-center justify-center w-14 h-14 bg-gradient-to-br ${theme.gradient} text-white rounded-full shadow-2xl ${isDarkMode ? 'shadow-black/50' : theme.shadow} hover:brightness-110 hover:-translate-y-1 transition-all duration-300`} 
