...
const App = () => {
  ...
  const formatDateTime = useCallback((dateString) => {
    return `${new Date(dateString).toLocaleDateString('id-ID', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} • ${new Date(dateString).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}`; 
  }, []);
  ...
  const handleEditEntry = useCallback((entry) => {
    setEditingId(entry.id); setNewTitle(entry.title); setNewContent(entry.content); setNewMood(entry.mood); setNewEntryLabels(entry.labels || []); setNewPaper(entry.paper || 'paper-polos'); setNewFont(entry.font || 'font-sans'); setCurrentView('write');
  }, []);
  ...
  const resetForm = useCallback(() => { 
    setEditingId(null); setNewTitle(''); setNewContent(''); setNewMood('senang'); setNewEntryLabels([]); setNewPaper('paper-polos'); setNewFont('font-sans');
  }, []);
  ...
  const handleDragEnd = useCallback((e) => {
    ...
    if (dragItem.current !== null && dragOverItem.current !== null) {
      ...
      copy.splice(dragItem.current, 1); copy.splice(dragOverItem.current, 0, content); setUserQuotes(copy);
    }
  ...
  const handleAddInstantLabel = useCallback(() => {
    ...
    setNewEntryLabels([...newEntryLabels, newId]); setInstantLabelName(''); setIsAddingInstantLabel(false);
  ...
  const getPaperStyle = (paperType) => {
    const base = { padding: '1rem', backgroundAttachment: 'local' };
    const lc = 'rgba(220, 150, 180, 0.4)', dc = 'rgba(220, 150, 180, 0.6)', gc = 'rgba(220, 150, 180, 0.3)';
    switch(paperType) {
      case 'paper-garis': return { ...base, backgroundImage: `linear-gradient(transparent calc(1.6em - 1px), ${lc} calc(1.6em - 1px), ${lc} 1.6em, transparent 1.6em)`, backgroundSize: '100% 2em', backgroundPosition: '0 1rem', lineHeight: '2em' };
      case 'paper-titik': return { ...base, backgroundImage: `radial-gradient(${dc} 1.5px, transparent 1.5px)`, backgroundSize: '2em 2em', backgroundPosition: '1rem calc(1rem + 0.6em)', lineHeight: '2em' };
      case 'paper-grid': return { ...base, backgroundImage: `linear-gradient(to right, ${gc} 1px, transparent 1px), linear-gradient(to bottom, transparent calc(1.6em - 1px), ${gc} calc(1.6em - 1px), ${gc} 1.6em, transparent 1.6em)`, backgroundSize: '2em 2em', backgroundPosition: '1rem 1rem', lineHeight: '2em' };
      default: return { ...base, backgroundColor: 'transparent', lineHeight: '1.75em' };
    }
  };
      ...
      <div className={`fixed top-0 left-0 h-full w-64 ${mode.bgSidebar} backdrop-blur-xl shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-r ${mode.border} ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="space-y-3">
            {[ { id: 'home', icon: <CalendarHeart size={20}/>, label: 'Semua Catatan', active: currentView === 'home' && !selectedLabelFilter }, { id: 'labels', icon: <Tag size={20}/>, label: 'Koleksi Label', active: currentView === 'labels' }, { id: 'quotes', icon: <Quote size={20}/>, label: 'Atur Quote', active: currentView === 'quotes' }].map(item => (
      ...
      <div className={`max-w-md mx-auto h-full ${isDarkMode ? 'bg-black/20' : 'bg-white/40'} backdrop-blur-sm shadow-xl ${isDarkMode ? 'shadow-black/50' : theme.shadow + '/50'} relative z-10 flex flex-col transition-colors duration-500`}>
        <header className="pt-10 pb-6 px-6 text-center relative flex justify-center items-center shrink-0">
          ...
          <div className="flex flex-col items-center">
            ...
            <p className={`text-[10px] ${mode.textMuted} font-medium tracking-widest uppercase mt-1`}>{currentView === 'trash' ? 'Tempat Sampah' : currentView === 'quotes' ? 'Koleksi Inspirasi' : currentView === 'labels' ? 'Manajemen Label' : currentView === 'settings' ? 'Pengaturan' : 'Ruang Cerita & Doa'}</p>
          </div>
        </header>
        ...
        <main className="flex-1 px-6 pb-24 overflow-y-auto">
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col pb-6">
                ...
                <div className={`flex justify-center gap-1 sm:gap-3 mb-3 pb-3 border-b ${mode.border}`}>
                  {moods.map(mood => (
                    <button key={mood.id} onClick={() => setNewMood(mood.id)} className={`flex flex-col items-center gap-1 p-2 rounded-xl transition-all ${newMood === mood.id ? `${isDarkMode ? 'bg-gray-700' : theme.bgMedium} scale-110 shadow-sm ring-2 ${theme.ring}` : `opacity-70 hover:opacity-100 ${mode.bgHover}`}`}>
                      <span className="text-2xl">{mood.emoji}</span><span className={`text-[10px] ${mode.textMuted} font-medium hidden sm:block`}>{mood.label}</span>
                    </button>
                  ))}
                </div>
                <div className="px-1">
              ...
              <div className="flex flex-wrap gap-4 px-1 shrink-0">
                <div className="flex items-center gap-2">
                  <FileText size={14} className={theme.textLight} /><span className={`text-[10px] font-bold ${mode.textMuted} uppercase tracking-widest`}>Kertas:</span>
                  <select value={newPaper} onChange={e => setNewPaper(e.target.value)} className={`text-xs font-medium ${mode.bgItem} ${mode.textMain} border ${mode.border} rounded-lg py-1 px-2 outline-none focus:ring-2 ${theme.ring} cursor-pointer`}>
                    <option value="paper-polos">Polos Putih</option><option value="paper-garis">Garis-Garis</option><option value="paper-titik">Titik-Titik</option><option value="paper-grid">Kotak-Kotak</option>
                  </select>
                </div>
                <div className="flex items-center gap-2">
                  <Type size={14} className={theme.textLight} /><span className={`text-[10px] font-bold ${mode.textMuted} uppercase tracking-widest`}>Font:</span>
                  <select value={newFont} onChange={e => setNewFont(e.target.value)} className={`text-xs font-medium ${mode.bgItem} ${mode.textMain} border ${mode.border} rounded-lg py-1 px-2 outline-none focus:ring-2 ${theme.ring} cursor-pointer`}>
                    <option value="font-sans">Modern</option><option value="font-elegant">Elegan</option><option value="font-handwriting">Tulis Tangan</option><option value="font-script">Latin Sambung</option>
                  </select>
                </div>
              </div>
              <div className={`flex-1 flex flex-col ${mode.bgInput} rounded-3xl shadow-inner border ${mode.border} min-h-[300px] overflow-hidden`}>
                ...
                <div ref={editorRef} contentEditable onInput={e => setNewContent(e.currentTarget.innerHTML)} style={{ fontSize: `${fontSize}px`, ...getPaperStyle(newPaper) }} className={`w-full flex-1 outline-none ${mode.textCard} overflow-y-auto transition-all ${newFont || 'font-sans'}`} placeholder="Ceritakan apa yang kamu rasakan hari ini..."></div>
              </div>
          ...
          {currentView === 'read' && activeEntry && (
            <div className={`${mode.bgCard} backdrop-blur-md rounded-3xl shadow-sm border ${mode.border} animate-in zoom-in-95 duration-300 overflow-hidden flex flex-col`}>
              ...
              <div className={`${mode.textCard} pb-12 transition-all ${activeEntry.font || 'font-sans'}`} style={{ fontSize: `${fontSize}px`, ...getPaperStyle(activeEntry.paper || 'paper-polos') }} dangerouslySetInnerHTML={{ __html: activeEntry.content }} />
            </div>
          )}
          ...
          {currentView === 'labels' && (
            <div className="space-y-6 animate-in fade-in duration-300 pb-10">
              <div className={`${mode.bgCard} p-5 rounded-3xl shadow-sm border ${mode.border}`}>
                ...
                <div className="flex gap-2">
                  <input type="text" placeholder="Nama label (misal: Sedih...)" value={newLabelName} onChange={e => setNewLabelName(e.target.value)} className={`flex-1 ${mode.bgInput} p-3 rounded-xl shadow-inner border ${mode.border} outline-none focus:ring-2 ${theme.ring} text-sm ${mode.textMain}`} />
                  ...
                  {deletedEntries.map(entry => (
                    <div key={entry.id} className={`w-full ${mode.bgInput} backdrop-blur-md rounded-2xl shadow-sm border ${mode.border} overflow-hidden opacity-80 hover:opacity-100 transition-opacity`}>
                      <div className="p-5 opacity-70">
                        ...
                        <div className={`text-sm ${mode.textMuted} line-clamp-1 italic ${entry.font || 'font-sans'}`} dangerouslySetInnerHTML={{ __html: entry.content }}></div>
                      </div>
