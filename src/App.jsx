...
// Definisi 5 Tema Warna Estetik
const themeColors = {
  pink: {
    name: "Sakura Pastel",
    gradient: "from-pink-500 to-purple-400",
    gradientLight: "from-pink-100 to-purple-100",
    text: "text-pink-600",
    textLight: "text-pink-400",
    bgLight: "bg-pink-50",
    bgMedium: "bg-pink-100",
    blob1: "bg-pink-200/40", blob2: "bg-purple-200/30", blob3: "bg-rose-100/40",
  ...
  blue: {
    ...
    blob1: "bg-blue-200/40", blob2: "bg-cyan-200/30", blob3: "bg-sky-100/40",
  ...
  green: {
    ...
    blob1: "bg-emerald-200/40", blob2: "bg-teal-200/30", blob3: "bg-green-100/40",
  ...
  orange: {
    ...
    gradientLight: "from-orange-100 to-rose-100",
    text: "text-orange-600",
    textLight: "text-orange-400",
    bgLight: "bg-orange-50",
    bgMedium: "bg-orange-100",
    blob1: "bg-orange-200/40", blob2: "bg-rose-200/30", blob3: "bg-amber-100/40",
  ...
  purple: {
    ...
    blob1: "bg-purple-200/40", blob2: "bg-indigo-200/30", blob3: "bg-violet-100/40",
...
const App = () => {
  ...
  // States UI & Form
  const [currentView, setCurrentView] = useState('home'); // home, write, read, trash, quotes, labels, settings
  ...
  const formatDateTime = (dateString) => {
    ...
    return `${date.toLocaleDateString('id-ID', optionsDate)} • ${date.toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}`; 
  };
  ...
  const handleDeleteEntry = (entry) => { setEntries(entries.filter(e => e.id !== entry.id)); setDeletedEntries([entry, ...deletedEntries]); };
  const handleRestoreEntry = (entry) => { setDeletedEntries(deletedEntries.filter(e => e.id !== entry.id)); setEntries([...entries, entry].sort((a, b) => new Date(b.date) - new Date(a.date))); };
  const resetForm = () => { setEditingId(null); setNewTitle(''); setNewContent(''); setNewMood('senang'); setNewEntryLabels([]); };
  ...
  const handleDeleteQuote = (index) => { const updated = [...userQuotes]; updated.splice(index, 1); setUserQuotes(updated); };
    ...
    return (
      <div className="flex flex-wrap gap-1.5 mt-2 mb-1">
        {entryLabelsArray.map(labelId => {
          ...
          return l ? (
            <span key={labelId} className={`text-[10px] border ${mode.border} px-2 py-0.5 rounded-full font-medium ${isDarkMode ? 'bg-gray-800 text-gray-300' : theme.bgLight + ' ' + theme.text}`}>
              {l.name}
            </span>
          ) : null;
        })}
  ...
  return (
    <div className={`h-[100dvh] w-full ${mode.bgMain} font-sans ${mode.textMain} relative overflow-hidden transition-colors duration-500 selection:${theme.bgMedium} selection:${theme.text}`}>
      ...
      {/* Efek Latar Belakang Cat Air (Redup saat Dark Mode) */}
      ...
      <div className={`fixed top-0 left-0 h-full w-64 ${mode.bgSidebar} backdrop-blur-xl shadow-2xl z-50 transform transition-transform duration-300 ease-in-out border-r ${mode.border} ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} flex flex-col`}>
        <div className="p-6 overflow-y-auto flex-1">
          ...
          <div className="space-y-3">
            <button onClick={() => { setCurrentView('home'); setSelectedLabelFilter(null); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 p-4 rounded-2xl font-medium transition-all ${currentView === 'home' && !selectedLabelFilter ? `bg-gradient-to-r ${isDarkMode ? 'from-gray-800 to-gray-700' : theme.gradientLight} ${theme.text} shadow-sm` : `${mode.bgHover} ${mode.textMuted}`}`}>
              <CalendarHeart size={20} /> Semua Catatan
            </button>
            <button onClick={() => { setCurrentView('labels'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 p-4 rounded-2xl font-medium transition-all ${currentView === 'labels' ? `bg-gradient-to-r ${isDarkMode ? 'from-gray-800 to-gray-700' : theme.gradientLight} ${theme.text} shadow-sm` : `${mode.bgHover} ${mode.textMuted}`}`}>
              <Tag size={20} /> Koleksi Label
            </button>
            <button onClick={() => { setCurrentView('quotes'); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-3 p-4 rounded-2xl font-medium transition-all ${currentView === 'quotes' ? `bg-gradient-to-r ${isDarkMode ? 'from-gray-800 to-gray-700' : theme.gradientLight} ${theme.text} shadow-sm` : `${mode.bgHover} ${mode.textMuted}`}`}>
      ...
      <div className={`max-w-md mx-auto h-full ${isDarkMode ? 'bg-black/20' : 'bg-white/40'} backdrop-blur-sm shadow-xl ${isDarkMode ? 'shadow-black/50' : theme.shadow + '/50'} relative z-10 flex flex-col transition-colors duration-500`}>
        ...
        <header className="pt-10 pb-6 px-6 text-center relative flex justify-center items-center shrink-0">
          {currentView === 'home' ? (
            <button onClick={() => setIsSidebarOpen(true)} className={`absolute left-6 p-2 ${mode.bgItem} rounded-full ${mode.bgHover} ${theme.text} transition-colors shadow-sm`}><Menu size={20} /></button>
          ) : (
            <button onClick={currentView === 'write' ? () => { resetForm(); setCurrentView('home'); } : () => setCurrentView('home')} className={`absolute left-6 p-2 ${mode.bgItem} rounded-full ${mode.bgHover} ${theme.text} transition-colors shadow-sm`}><ChevronLeft size={20} /></button>
          )}
        ...
        <main className="flex-1 px-6 pb-24 overflow-y-auto">
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4 duration-300 h-full flex flex-col pb-6">
                  ...
                  <div className="flex flex-wrap gap-2 items-center">
                    {labels.map(label => (
                      <button
                        key={label.id}
                        onClick={() => {
                          if (newEntryLabels.includes(label.id)) setNewEntryLabels(newEntryLabels.filter(id => id !== label.id));
                          else setNewEntryLabels([...newEntryLabels, label.id]);
                        }}
              ...
              <div className={`flex-1 flex flex-col ${mode.bgInput} p-4 rounded-3xl shadow-inner border ${mode.border} min-h-[300px]`}>
                <input
                  ...
                  className={`w-full bg-transparent text-xl font-bold ${mode.textMain} placeholder-gray-400 border-b-2 ${mode.border} focus:border-transparent focus:ring-0 outline-none pb-3 mb-4 transition-colors shrink-0`}
                />
                ...
                <div
                  ...
                  className={`w-full flex-1 outline-none ${mode.textCard} leading-relaxed overflow-y-auto`}
          ...
          {currentView === 'read' && activeEntry && (
            <div className={`${mode.bgCard} backdrop-blur-md rounded-3xl p-6 shadow-sm border ${mode.border} animate-in zoom-in-95 duration-300`}>
              ...
              <div 
                className={`${mode.textCard} leading-loose whitespace-pre-wrap break-words pb-8`} 
                style={{ fontSize: `${fontSize}px` }}
                  ...
                  <div className="space-y-3">
                    {userQuotes.map((quote, index) => (
                      <div key={quote.id || index} draggable onDragStart={(e) => handleDragStart(e, index)} onDragEnter={(e) => handleDragEnter(e, index)} onDragEnd={handleDragEnd} onDragOver={(e) => e.preventDefault()} className={`${mode.bgCard} backdrop-blur-sm p-4 rounded-2xl shadow-sm border ${mode.border} flex justify-between items-start gap-3 group cursor-move hover:shadow-md transition-all`}>
                        ...
                        <button onClick={() => handleDeleteQuote(index)} className={`${mode.textMuted} hover:text-red-500 transition-colors p-1 ${mode.bgItem} rounded-lg shadow-sm group-hover:opacity-100 opacity-50 cursor-pointer z-10 border ${mode.border}`} title="Hapus Kutipan"><Trash2 size={16} /></button>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className={`${mode.bgCard} p-5 rounded-3xl shadow-sm border ${mode.border}`}>
                ...
                <div className="flex gap-2">
                  <input
                    ...
                    className={`flex-1 ${mode.bgInput} p-3 rounded-xl shadow-inner border ${mode.border} outline-none focus:ring-2 ${theme.ring} text-sm ${mode.textMain}`}
                  />
                  <button
                    onClick={() => {
                    ...
                    className={`px-5 bg-gradient-to-r ${theme.gradient} text-white rounded-xl font-bold shadow-md transition-all disabled:opacity-50`}
                  >
                    Buat
                  </button>
                </div>
                        ...
                        <div key={label.id} className={`${mode.bgCard} backdrop-blur-sm p-4 rounded-2xl shadow-sm border ${mode.border} flex flex-col justify-between hover:shadow-md transition-all`}>
                          ...
                          <div className="mt-4 flex gap-2">
                            <button 
                              ...
                              className={`flex-1 ${isDarkMode ? 'bg-gray-800' : theme.bgLight} ${theme.text} border ${mode.border} py-1.5 rounded-lg text-xs font-bold hover:brightness-95 transition-all shadow-sm`}
                            >
                              Lihat
                            </button>
                            <button 
                              onClick={() => setLabels(labels.filter(l => l.id !== label.id))}
                              className={`p-1.5 ${mode.textMuted} ${mode.bgItem} hover:text-red-500 rounded-lg shadow-sm transition-colors border ${mode.border}`}
                              title="Hapus Label"
                            >
                              <Trash2 size={14} />
                            </button>
          ...
          {currentView === 'trash' && (
            <div className="space-y-4 animate-in fade-in duration-300 pb-10">
              {deletedEntries.length === 0 ? (
                <div className={`text-center py-20 ${mode.textMuted} flex flex-col items-center`}>
                  <ArchiveX size={40} className={`mb-3 ${theme.textLight} opacity-50`} /><p>Tempat sampah kosong.</p>
                </div>
              ) : (
                <>
                  ...
                  {deletedEntries.map((entry) => (
                    <div key={entry.id} className={`w-full ${mode.bgInput} backdrop-blur-md rounded-2xl shadow-sm border ${mode.border} overflow-hidden opacity-80 hover:opacity-100 transition-opacity`}>
                      <div className="p-5 opacity-70">
                        ...
                        <div className={`flex items-center gap-1.5 text-xs ${mode.textMuted} mb-2`}>
                          {getTimeIcon(entry.date)}<span>{formatDateTime(entry.date)}</span>
                        </div>
            ...
            <div className="space-y-6 animate-in fade-in duration-300 pb-10">
              <div className={`${mode.bgCard} p-6 rounded-3xl shadow-sm border ${mode.border}`}>
                ...
                <div className={`flex items-center justify-between p-4 rounded-2xl ${mode.bgInput} border ${mode.border} mb-6`}>
                  <span className={`font-bold ${mode.textMain} flex items-center gap-2`}>
                    {isDarkMode ? <Moon size={18} className="text-indigo-400"/> : <Sun size={18} className="text-amber-500"/>}
                    Mode Gelap
                  </span>
                  <button 
                    onClick={() => setIsDarkMode(!isDarkMode)} 
                    className={`w-14 h-7 rounded-full flex items-center p-1 transition-colors duration-300 ${isDarkMode ? 'bg-indigo-500' : 'bg-gray-300'}`}
                  >
